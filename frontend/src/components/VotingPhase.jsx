  ...
  return (
    <div className="phase voting-phase">
      ...
      <p className="voting-instruction">
        Tente adivinhar quem escreveu cada resposta!
      </p>
      <div className="answers-container">
        {(answers || [])
          ...
          .map((ans) => {
            ...
            const alreadyVotedPlayers = Object.entries(
              hasSubmittedVotes
                ? gameState.votedGuesses?.[currentPlayer?.id] || {}
                : localVotes,
            )
                    ...
                    .map((p) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            if (myVote === p.id) {
                              // Desfazer voto ao clicar novamente
                              setLocalVotes((prev) => {
                                const updated = { ...prev };
                                delete updated[ans.id];
                                return updated;
                              });
                            } else if (!isAlreadyVoted) {
                              // Votar em um novo jogador
                              setLocalVotes((prev) => ({
                                ...prev,
                                [ans.id]: p.id,
                              }));
                          ...
                          disabled={
                            hasSubmittedVotes ||
                            (isAlreadyVoted && myVote !== p.id)
                          }
                          title={
                            myVote === p.id
                              ? "Clique novamente para desfazer o voto"
                              : isAlreadyVoted && myVote !== p.id
                                ? "Você já votou nesta pessoa em outra resposta"
                                : ""
                          }
                        >
                          {p.name}
      ...
      {hasSubmittedVotes ? (
        <div className="answer-sent">
          Votos enviados! Aguardando outros jogadores...
        </div>
      ) : (
        <button
          ...
          disabled={
            Object.keys(localVotes).length <
            (answers?.filter((a) => a.realId !== currentPlayer?.id).length || 0)
          }
      ...
      <div className="progress-bar">
        <p>
          Votaram: {Object.keys(gameState.votedGuesses || {}).length}/
          {totalPlayers}
        </p>
        <div
          className="bar-fill"
          style={{
            width: `${totalPlayers ? (Object.keys(gameState.votedGuesses || {}).length / totalPlayers) * 100 : 0}%`,
          }}
        ></div>
      </div>
    </div>
